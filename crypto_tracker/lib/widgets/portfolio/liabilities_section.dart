...
class _LiabilitiesSectionState extends State<LiabilitiesSection> {
  ...
  void hideShow() {
    if (ShowNotifier().show) {
      _liabilitySum = NumberFormat.simpleCurrency(decimalDigits: 0)
          .format(LiabilityObserver.instance.liabilitySum);
      _specificLiabilityAmounts = LiabilityObserver.instance.specificLiabilityAmounts.map(
          (key, value) =>
              MapEntry(key, NumberFormat.simpleCurrency(decimalDigits: 0).format(value)));
      _specificLiabilityPercentages = LiabilityObserver.instance.specificLiabilityPercentages
          .map((key, value) => MapEntry(key, value.toString()));
    } else {
      String char = '\u2731';
      _liabilitySum = char * 4;
      _specificLiabilityAmounts = LiabilityObserver.instance.specificLiabilityAmounts
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        SectionTitle(isAssets: false, leading: 'Liabilities', trailing: _liabilitySum),
        if (displayPercentage)
          for (MapEntry<String, String> entry in _specificLiabilityPercentages.entries)
            ListItem(
                leading: Icon(CustomIcons.getLiabilityIcon(entry.key)),
                title: entry.key,
                trailing: entry.value + (ShowNotifier().show ? '%' : ''),
                trailingAlternative: WithdrawButton(assetType: getLiabilityTypeFromName(entry.key), statementType: StatementType.LIABILITY),
